...
// --- Course Icons (moved from data.js — JSX must live in main.js) ---
const CourseIcon = ({ type, colorClass }) => {
    const cls = `h-8 w-8 mb-4 ${colorClass}`;
...
const About = () => (
    <Section id="about" title="About Me">
        <div className="bg-gray-800/50 backdrop-blur-md p-8 rounded-2xl shadow-xl border border-gray-700/50 max-w-4xl mx-auto">
            <p className="text-lg text-gray-300 text-center leading-relaxed whitespace-pre-line">
                {portfolioData.about}
            </p>
        </div>
    </Section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {portfolioData.projects.map(project => (
                <a
                    ...
                    key={project.id}
                    className="group block bg-gray-800/50 backdrop-blur-md rounded-xl overflow-hidden border border-gray-700/50 hover:border-sky-500/50 transition-all duration-300 transform hover:-translate-y-2 shadow-lg hover:shadow-sky-500/20 h-full flex flex-col"
                >
                    <div className="p-8 flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            {project.inProgress ? (
                                <span className="shrink-0 text-xs font-bold bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 px-2 py-1 rounded-full whitespace-nowrap">
                                    🚧 {project.expectedDate}
                                </span>
                            ) : (
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-gray-500 group-hover:text-sky-400 transition-colors">
